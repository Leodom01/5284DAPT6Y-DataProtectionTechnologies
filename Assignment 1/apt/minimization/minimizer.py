...
class GeneralizeToRepresentative(BaseEstimator, MetaEstimatorMixin, TransformerMixin):
    ...
    def fit(self, X: Optional[DATA_PANDAS_NUMPY_TYPE] = None, y: Optional[DATA_PANDAS_NUMPY_TYPE] = None,
        ...
        if self.estimator and dataset and dataset.get_samples() is not None and dataset.get_labels() is not None:
            ...
            # We have to smartly decide which are the best features to minimize
            ...
            if accuracy > self.target_accuracy:
                ...
                while accuracy > self.target_accuracy:
                    ...
                    if accuracy < self.target_accuracy:
                        ...
                        # Those where bothersome, I don't want to show internal data to the user
                        #print('Pruned tree to level: %d, new relative accuracy: %f' % (self._level, accuracy))
                        #print("Current generalizations: ", self._generalizations)
                        self._level += 1
    ...
    def _get_feature_to_remove(self, original_data, prepared_data, nodes, labels, feature_data, current_accuracy,
        ...
        min_ncp_value = sys.float_info.max
        min_ncp_feature = None
        # A list that contains all NCP values and the corresponding feature
        ncp_values = []
        ...
        for feature in ranges.keys():
            if feature not in self._generalizations['untouched']:
                ...
                ncp_values.append({'value': feature_ncp, 'feature': feature})
        ...
        for feature in categories.keys():
            if feature not in self.generalizations['untouched']:
                ...
                min_ncp_entry = min(ncp_values, key=lambda x: x['value'])
                # this entry is actually not needed now but will be useful in the future
                # min_ncp_value = min_ncp_entry['value']
                min_ncp_feature = min_ncp_entry['feature']

        print('feature to remove: ' + (str(min_ncp_feature) if min_ncp_feature is not None else 'none'))
        return min_ncp_feature
